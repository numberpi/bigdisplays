...
class Setup:
	...
	def __init__(self, device=None, camres=(640,480), disptype='window', dispres=(1024,768), display=None):
		...
		self.settings = {'pupilcol':(0,0,0), \
					...
					'pupilrect':pygame.Rect(camres[0]/2-50,camres[1]/2-25,50,50), \
					'pupilbounds': [0,0,0,0], \
					'':None					
					}
	...
	def run_GUI(self):
		...
		stagevars[2]['prectsize'] = 50,50 # pupilrectsize
...
class CamEyeTracker:
	...
	def __init__(self, device=None, camres=(640,480)):
		...
		self.settings = {'pupilcol':(0,0,0), \
					...
					'nonthresholdcol':(100,100,255,255), \
					'pupilpos':(-1,-1), \
					'pupilrect':pygame.Rect(self.camres[0]/2-50,self.camres[1]/2-25,50,50), \
					'pupilbounds': [0,0,0,0], \
					'':None					
					}
	...
	def find_pupil(self, thresholded, pupilrect=False):
		...
		xPositionSum += self.settings['pupilpos'][0]
		yPositionSum += self.settings['pupilpos'][1]
		Count += 1

		if time.time() - OldTime > TimeDelta:
			sys.stdout.write(str(xPositionSum / Count))
			sys.stdout.write(';')
			sys.stdout.write(str(yPositionSum / Count))
			sys.stdout.write('\n')
			sys.stdout.flush()
			OldTime = time.time()
			Count = 0
